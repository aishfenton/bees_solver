package com.visfleet.beessolver.vrp.data

import com.visfleet.beessolver.vrp.Location
import com.visfleet.beessolver.vrp.Job

// Best 
// 
// 187 274 868.29298541 0.00 0.00 (RR, RI, 50,5,2, Age Search Rampup, KO, 0.2 destroy)
// COMMENT : (Christophides and Eilon)
object P08D101K09 extends Problem {
  
  def maxVehicles = 9
  def maxCapacity = 200
  def maxRouteTime = 230
  val serviceTime = 10
  def depot = new Location(35, 35)
  
  def jobs = ImportUtil.toJobsWithoutId(List(
    (41, 49, 10),
    (35, 17, 7),
    (55, 45, 13),
    (55, 20, 19),
    (15, 30, 26),
    (25, 30, 3),
    (20, 50, 5),
    (10, 43, 9),
    (55, 60, 16),
    (30, 60, 16),
    (20, 65, 12),
    (50, 35, 19),
    (30, 25, 23),
    (15, 10, 20),
    (30, 5, 8),
    (10, 20, 19),
    (5, 30, 2),
    (20, 40, 12),
    (15, 60, 17),
    (45, 65, 9),
    (45, 20, 11),
    (45, 10, 18),
    (55, 5, 29),
    (65, 35, 3),
    (65, 20, 6),
    (45, 30, 17),
    (35, 40, 16),
    (41, 37, 16),
    (64, 42, 9),
    (40, 60, 21),
    (31, 52, 27),
    (35, 69, 23),
    (53, 52, 11),
    (65, 55, 14),
    (63, 65, 8),
    (2, 60, 5),
    (20, 20, 8),
    (5, 5, 16),
    (60, 12, 31),
    (40, 25, 9),
    (42, 7, 5),
    (24, 12, 5),
    (23, 3, 7),
    (11, 14, 18),
    (6, 38, 16),
    (2, 48, 1),
    (8, 56, 27),
    (13, 52, 36),
    (6, 68, 30),
    (47, 47, 13),
    (49, 58, 10),
    (27, 43, 9),
    (37, 31, 14),
    (57, 29, 18),
    (63, 23, 2),
    (53, 12, 6),
    (32, 12, 7),
    (36, 26, 18),
    (21, 24, 28),
    (17, 34, 3),
    (12, 24, 13),
    (24, 58, 19),
    (27, 69, 10),
    (15, 77, 9),
    (62, 77, 20),
    (49, 73, 25),
    (67, 5, 25),
    (56, 39, 36),
    (37, 47, 6),
    (37, 56, 5),
    (57, 68, 15),
    (47, 16, 25),
    (44, 17, 9),
    (46, 13, 8),
    (49, 11, 18),
    (49, 42, 13),
    (53, 43, 14),
    (61, 52, 3),
    (57, 48, 23),
    (56, 37, 6),
    (55, 54, 26),
    (15, 47, 16),
    (14, 37, 11),
    (11, 31, 7),
    (16, 22, 41),
    (4, 18, 35),
    (28, 18, 26),
    (26, 52, 9),
    (26, 35, 15),
    (31, 67, 3),
    (15, 19, 1),
    (22, 22, 2),
    (18, 24, 22),
    (26, 27, 27),
    (25, 24, 20),
    (22, 27, 11),
    (25, 21, 12),
    (19, 21, 10),
    (20, 26, 9),
    (18, 18, 17)
  ), serviceTime)

  def bestKnownAnswer = 865.94

}
